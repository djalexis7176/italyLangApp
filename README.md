...
This build is **27**. The file is named `index-v27-number-range.html` — **rename it to `index.html` before or after uploading**, because GitHub Pages serves `index.html` as the front page. The version sits at the right of the blue bar at the top of every screen — `v27 · 8.31.26`. Once the app has checked GitHub it adds **✓ latest** in green; if a newer build is waiting it turns gold and says **update ready**; with no signal it says **offline**. Tap the stamp any time to check. The Plan page repeats it in full under *This app*, with the publish date and a list of what changed in each build.
...
1. In `index.html`, near the top of the script: `const BUILD = "27";` — and add a line to the `CHANGES` list describing what you did.
2. In `sw.js`, line 5: `const BUILD = "27";`
...
**Build your own range** sits at the top: pick a start and an end, count in ones, twos, fives, tens, hundreds or anything you type, and go up, down or shuffled. A preview shows how many numbers that is, the first few spelled out, and roughly how long it takes to read aloud at your current pause. Then run it any of five ways — say them, listen and pick, write them out, have them read to you hands-free, or hear each one inside a real sentence (*Camera quarantasei · Il binario è il sette · Sono ventidue chilometri*). The last range you used is remembered.

Sessions cap at 300 numbers, so 1 to 1000 by ones is trimmed with a nudge to raise the step. In Listen and pick the wrong answers are near misses drawn from the same range, so 47 competes with 46, 48 and 45 rather than something obvious.

An accordion gives the seven rules — why *ventuno* and *ventotto* lose a letter, why *ventitré* takes an accent, why *cento* never takes *un*, why *mille* becomes *mila* — with 0 to 20 laid out and spoken.
...
**Practice → Conversations** holds six scripts set around Bassano del Grappa, with your own details written into them from the sheet: your name, where you're from, how you're related to the couple, who you're travelling with, your departure date, where you go next, and which wine and beer you said you liked.

Meeting a guest at the wedding · Who's who at the table · Asking a local what to do · At the winery · At the taproom · Bumping into them again.

Three ways through each: **Answer & check** hides your lines until you've tried them, **Read it all** shows everything, and **↻ Repeat after me** runs the whole script hands-free with the pause settings.

The places are real — CRAK at Campodarsego and its Guerrilla IPA, Contrà Soarda in the San Michele hills and its Vespaiolo, Marostica's chess square, the Ponte degli Alpini. Distances are given as rough drive times rather than opening hours, which change.

## Directions

**Phrases → Quick reference → Directions, both ways** covers the answer as well as the question: *Vada sempre dritto · Giri a destra al semaforo · Prenda la seconda a destra · Alla rotonda prenda la terza uscita · Segua le indicazioni per Marostica · È a circa cinquecento metri · In macchina sono quaranta minuti · Non si può sbagliare.* Thirty phrases, with the road vocabulary added to the dictionary and a **vada vs vai** drill on formal and informal commands.
